...
const TEST_TYPES = [
  ['temperature','Temperature','°F','#ef5f59'],['salinity','Salinity','ppt','#5b9cf0'],['ph','pH','','#44c9c0'],['nitrate','Nitrate','ppm','#f29b45'],
  ['phosphorus','Phosphorus','ppb','#a881e6'],['nitrite','Nitrite','ppm','#ef744b'],['ammonia','Ammonia','ppm','#d8bd3f'],['magnesium','Magnesium','ppm','#64b87a']
];
...
function normalizeData(raw){
  ...
  base.fishEntries=(Array.isArray(source.fishEntries)?source.fishEntries:[]).slice(0,10000).map(v=>({id:safeId(v.id),fish:FISH_TABS.some(x=>x[0]===v.fish)?v.fish:FISH_TABS[0][0],kind:['feeding','behavior'].includes(v.kind)?v.kind:'behavior',date:safeDate(v.date)||today(),time:safeTime(v.time),text:safeText(v.text,2000),photos:Array.isArray(v.photos)?v.photos.slice(0,20):[],createdAt:safeText(v.createdAt,40)||timestamp()})).filter(v=>v.text);
  base.corals=(Array.isArray(source.corals)?source.corals:[]).slice(0,1000).map(v=>({id:safeId(v.id),name:safeText(v.name,120),genus:safeText(v.genus,120),acquisitionDate:safeDate(v.acquisitionDate),notes:(Array.isArray(v.notes)?v.notes:[]).slice(0,1000).map(n=>({id:safeId(n.id),date:safeDate(n.date)||today(),text:safeText(n.text,4000),createdAt:safeText(n.createdAt,40)||timestamp()})).filter(n=>n.text),photos:Array.isArray(v.photos)?v.photos.slice(0,50):[],createdAt:safeText(v.createdAt,40)||timestamp()})).filter(v=>v.name);
  base.tankVisual=(Array.isArray(source.tankVisual)?source.tankVisual:[]).slice(0,5000).map(v=>({id:safeId(v.id),date:safeDate(v.date)||today(),notes:safeText(v.notes,2000),photos:Array.isArray(v.photos)?v.photos.slice(0,50):[],createdAt:safeText(v.createdAt,40)||timestamp()}));
  base.goals=(Array.isArray(source.goals)?source.goals:[]).slice(0,2000).map(v=>({id:safeId(v.id),title:safeText(v.title,200),notes:safeText(v.notes,3000),targetDate:safeDate(v.targetDate),photos:Array.isArray(v.photos)?v.photos.slice(0,50):[],createdAt:safeText(v.createdAt,40)||timestamp()})).filter(v=>v.title);
  base.settings={schema:1}; return base;
}
...
function renderTesting(){
  ...
  main.innerHTML=`<section class="screen stack">${screenHead('Testing','One measurement at a time.')}
    ...
    <div class="panel"><div class="toolbar" style="justify-content:space-between"><strong>${esc(meta[1])} history</strong><div class="chips">${[['3m','3 Months'],['1y','1 Year'],['3y','3 Years']].map(([r,l])=>`<button class="chip ${r===testRange?'active':''}" data-range="${r}">${l}</button>`).join('')}</div></div>${chartSvg(rows,meta)}</div>
...
function chartSvg(rows,meta){const days={"3m":92,"1y":366,"3y":1096}[testRange],cut=Date.now()-days*86400000,points=rows.filter(r=>new Date(r.date+'T12:00:00').getTime()>=cut).sort((a,b)=>a.date.localeCompare(b.date));if(!points.length)return `<div class="empty" style="margin-top:14px">No readings in this range.</div>`;const vals=points.map(p=>p.value),min=Math.min(...vals),max=Math.max(...vals),span=max-min||1,w=760,h=220,pad=28,x=i=>pad+(points.length===1?(w-2*pad)/2:i*(w-2*pad)/(points.length-1)),y=v=>h-pad-(v-min)/span*(h-2*pad),coords=points.map((p,i)=>`${x(i)},${y(p.value)}`).join(' ');return `<svg class="chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(meta[1])} chart"><line class="gridline" x1="${pad}" y1="${pad}" x2="${pad}" y2="${h-pad}"/><line class="gridline" x1="${pad}" y1="${h-pad}" x2="${w-pad}" y2="${h-pad}"/><polyline class="series" stroke="${meta[3]}" points="${coords}"/>${points.map((p,i)=>`<circle class="dot" cx="${x(i)}" cy="${y(p.value)}" r="4" fill="${meta[3]}"><title>${esc(fmtDate(p.date))}: ${p.value}${meta[2]?' '+meta[2]:''}</title></circle>`).join('')}<text x="${pad}" y="16">${max}${meta[2]?' '+meta[2]:''}</text><text x="${pad}" y="${h-5}">${esc(fmtDate(points[0].date))}</text><text x="${w-pad}" text-anchor="end" y="${h-5}">${esc(fmtDate(points.at(-1).date))}</text></svg>`}
function testRow(r){const m=testMeta(r.type);return `<article class="row"><div class="row-copy"><strong>${esc(fmtDate(r.date))}</strong><span>${esc(m[1])}</span></div><div style="display:flex;align-items:center;gap:8px"><div class="row-value">${r.value}${m[2]?` <small>${esc(m[2])}</small>`:''}</div>${rowActions(r.id)}</div></article>`}
...
function waterChart(rows){const p=[...rows].sort((a,b)=>a.date.localeCompare(b.date));if(!p.length)return `<div class="empty" style="margin-top:14px">No water changes in the last 6 months.</div>`;const vals=p.map(x=>pct(x.gallons)),max=Math.max(10,...vals),w=760,h=220,pad=28,x=i=>pad+(p.length===1?(w-2*pad)/2:i*(w-2*pad)/(p.length-1)),y=v=>h-pad-v/max*(h-2*pad),coords=p.map((r,i)=>`${x(i)},${y(pct(r.gallons))}`).join(' ');return `<svg class="chart" viewBox="0 0 ${w} ${h}"><line class="gridline" x1="${pad}" y1="${pad}" x2="${pad}" y2="${h-pad}"/><line class="gridline" x1="${pad}" y1="${h-pad}" x2="${w-pad}" y2="${h-pad}"/><polyline class="series" stroke="#5b9cf0" points="${coords}"/>${p.map((r,i)=>`<circle class="dot" cx="${x(i)}" cy="${y(pct(r.gallons))}" r="4" fill="#5b9cf0"><title>${esc(fmtDate(r.date))}: ${pct(r.gallons)}%</title></circle>`).join('')}</svg>`}
function waterRow(r){return `<article class="row"><div class="row-copy"><strong>${esc(fmtDate(r.date))}</strong>${r.notes?`<span>${esc(r.notes)}</span>`:''}</div><div style="display:flex;align-items:center;gap:8px"><div class="row-value">${r.gallons} gal <small>· ${pct(r.gallons)}%</small></div>${rowActions(r.id)}</div></article>`}

function renderDosing(){const rows=[...data.doses].sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`));main.innerHTML=`<section class="screen stack">${screenHead('Dosing','A simple history of what was added and when.')} ${rows.length?`<div class="timeline">${rows.map(r=>`<article class="timeline-item"><time>${esc(fmtDate(r.date))}${r.time?` · ${esc(r.time)}`:''}</time><strong>${esc(r.additive)}${r.amount?` · ${esc(r.amount)}`:''}</strong>${r.notes?`<div class="muted">${esc(r.notes)}</div>`:''}<div class="row-actions" style="margin-top:6px">${rowActions(r.id)}</div></article>`).join('')}</div>`:empty('No dosing history','Add a dose when you add something to the tank.')}</section>`;wireRows('dose')}

function renderFish(){const label=FISH_TABS.find(x=>x[0]===fishTab)?.[1]||'',rows=data.fishEntries.filter(x=>x.fish===fishTab).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`));main.innerHTML=`<section class="screen stack">${screenHead('Fish','Feeding and behavior notes by animal.')}<div class="subtabs">${FISH_TABS.map(([id,l])=>`<button class="subtab ${id===fishTab?'active':''}" data-fish="${id}">${esc(l)}</button>`).join('')}</div><div class="list">${rows.length?rows.map(r=>`<article class="row"><div class="row-copy"><strong>${r.kind==='feeding'?'Feeding':'Behavior'} · ${esc(fmtDate(r.date))}${r.time?` · ${esc(r.time)}`:''}</strong><span>${esc(r.text)}</span></div>${rowActions(r.id)}</article>`).join(''):empty(`No ${label} entries`,'Add a feeding or behavior note. Photos will plug into these entries once Cloudinary Worker configuration is added.')}</div></section>`;main.querySelectorAll('[data-fish]').forEach(b=>b.onclick=()=>{fishTab=b.dataset.fish;renderFish()});wireRows('fish')}

function renderCoral(){if(!coralTab&&data.corals.length)coralTab=data.corals[0].id;const selected=data.corals.find(x=>x.id===coralTab);main.innerHTML=`<section class="screen stack">${screenHead('Coral','One editable subtab per coral or colony.')}<div class="subtabs"><button class="subtab ${!selected?'active':''}" data-coral-new>＋ Add coral</button>${data.corals.map(c=>`<button class="subtab ${c.id===coralTab?'active':''}" data-coral="${c.id}">${esc(c.name)}</button>`).join('')}</div>${selected?coralDetail(selected):empty('No coral yet','Create a coral subtab to track its details and dated notes.')}</section>`;main.querySelector('[data-coral-new]')?.addEventListener('click',()=>openEditor('coral'));main.querySelectorAll('[data-coral]').forEach(b=>b.onclick=()=>{coralTab=b.dataset.coral;renderCoral()});$('#editCoral')?.addEventListener('click',()=>openEditor('coral',selected));$('#addCoralNote')?.addEventListener('click',()=>openEditor('coralNote',selected));$('#deleteCoral')?.addEventListener('click',()=>deleteItem('coral',selected.id,true));}
function coralDetail(c){const notes=[...c.notes].sort((a,b)=>b.date.localeCompare(a.date)||b.createdAt.localeCompare(a.createdAt));return `<div class="grid"><section class="panel"><div class="screen-head"><div><h2>${esc(c.name)}</h2><p>${esc(c.genus||'Genus not set')}${c.acquisitionDate?` · Acquired ${esc(fmtDate(c.acquisitionDate))}`:''}</p></div></div><div class="toolbar"><button class="button secondary" id="editCoral">Edit coral</button><button class="button danger" id="deleteCoral">Delete coral</button></div><div class="photo-placeholder" style="margin-top:16px">Photos will use Cloudinary through your Worker.</div></section><section class="panel"><div class="screen-head"><div><h2>Notes</h2><p>Newest first</p></div><button class="button secondary" id="addCoralNote">Add Note</button></div><div class="notes-list">${notes.length?notes.map(n=>`<article class="note-card"><time>${esc(fmtDate(n.date))}</time><p>${esc(n.text)}</p></article>`).join(''):`<div class="muted">No notes yet.</div>`}</div></section></div>`}

function renderVisual(){const rows=[...data.tankVisual].sort((a,b)=>b.date.localeCompare(a.date));main.innerHTML=`<section class="screen stack">${screenHead('Tank Visual','A dated visual journal of the tank.')}<div class="photo-placeholder">Cloudinary image uploads are scaffolded but not enabled until the Worker endpoint is configured.</div><div class="list">${rows.length?rows.map(r=>`<article class="row"><div class="row-copy"><strong>${esc(fmtDate(r.date))}</strong><span>${esc(r.notes||'Tank visual entry')}</span></div>${rowActions(r.id)}</article>`).join(''):empty('No visual entries yet','Add a dated entry now; image upload will attach to these entries once configured.')}</div></section>`;wireRows('visual')}
function renderGoals(){const rows=[...data.goals].sort((a,b)=>(a.targetDate||'9999').localeCompare(b.targetDate||'9999')||a.createdAt.localeCompare(b.createdAt));main.innerHTML=`<section class="screen stack">${screenHead('Goals','Future ideas for the tank, kept intentionally simple.')}<div class="list">${rows.length?rows.map(r=>`<article class="row"><div class="row-copy"><strong>${esc(r.title)}</strong>${r.targetDate?`<span>Target ${esc(fmtDate(r.targetDate))}</span>`:''}${r.notes?`<small>${esc(r.notes)}</small>`:''}</div>${rowActions(r.id)}</article>`).join(''):empty('No goals yet','Add something you want to change, add, or achieve with the tank.')}</div></section>`;wireRows('goal')}
function rowActions(id){return `<div class="row-actions"><button class="row-action" data-edit="${esc(id)}" aria-label="Edit">✎</button><button class="row-action" data-delete="${esc(id)}" aria-label="Delete">×</button></div>`}
...
function closeButton(){return `<button type="button" class="icon-button dialog-close" data-close aria-label="Close">×</button>`}
...
editorForm.onsubmit=async e=>{e.preventDefault();const fd=new FormData(editorForm),kind=editorForm.dataset.kind,id=editorForm.dataset.id,now=timestamp();let collection='',record=null;
  ...
  if(kind==='coral'){collection='corals';const old=data.corals.find(x=>x.id===id);record={id,name:safeText(fd.get('name'),120),genus:safeText(fd.get('genus'),120),acquisitionDate:safeDate(fd.get('acquisitionDate')),notes:old?.notes||[],photos:old?.photos||[],createdAt:old?.createdAt||now};coralTab=id}
