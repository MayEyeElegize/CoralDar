...
const HOME_QUOTES_KEY = 'coraldar-home-quotes';
const DEFAULT_HOME_QUOTES = ['We believe that we invent symbols. The truth is that they invent us.'];
const HOME_QUOTE_SEED = Math.random();
...
const PHOTO_OWNERS = ['fish','coral','visual','goal','food'];
const ICONS = {
  ...
  coral:'M12 21V10M12 14 7 9M7 9 4 12M7 9 8 4M12 10 17 6M17 6 20 9M17 6 16 2M12 13 15 16M15 16 18 14',check:'M5 12.5l4.5 4.5L19 7.5',
...
const NAV = [
  ...
  ['fish','Fish',ICONS.fish],['coral','Coral',ICONS.coral],['visual','Tank','M3 5h18v14H3zM7 15l3-3 3 3 2-2 3 3M8 9h.01']
];
...
let data = emptyData();
let activeTab = 'home';
let testType = 'temperature', testRange = '3m', testChipScroll = 0, fishTab = '', coralTab = '', fishDetailOpen = false, coralDetailOpen = false, heroPhotos = [], heroIndex = 0, heroTimer = null;
...
function normalizeData(raw){
  ...
  base.fish=list('fish',500).map(v=>({id:safeId(v.id),name:safeText(v.name,80),acquisitionDate:safeDate(v.acquisitionDate),createdAt:created(v)})).filter(v=>v.name);
  base.foodTypes=list('foodTypes',500).map(v=>({id:safeId(v.id),name:safeText(v.name,120),createdAt:created(v)})).filter(v=>v.name);
  base.feedings=list('feedings',10000).map(v=>({id:safeId(v.id),foodTypeId:ID_PATTERN.test(String(v.foodTypeId||''))?String(v.foodTypeId):'',food:safeText(v.food,120),date:safeDate(v.date)||today(),time:safeTime(v.time),notes:safeText(v.notes,500),createdAt:created(v)})).filter(v=>v.food);
...
const thumbSrc = p => photoSize(p.url,'c_fill,g_center,w_320,h_320,q_auto,f_auto');
...
function photoOwnerLabel(p){if(p.owner==='fish')return data.fish.find(f=>f.id===p.ownerId)?.name||'Fish';if(p.owner==='coral')return data.corals.find(c=>c.id===p.ownerId)?.name||'Coral';if(p.owner==='goal')return data.goals.find(g=>g.id===p.ownerId)?.title||'Goal';if(p.owner==='food')return data.foodTypes.find(f=>f.id===p.ownerId)?.name||'Food';return 'Tank'}
...
function burstBubbles(button){if(reducedMotion.matches)return;const host=button.closest('.home-stat')||button.parentElement;for(let i=0;i<14;i++){const b=document.createElement('span');b.className='refill-bubble';b.style.setProperty('--x',`${-50+Math.random()*100}px`);b.style.setProperty('--drift',`${-24+Math.random()*48}px`);b.style.setProperty('--size',`${5+Math.random()*10}px`);b.style.setProperty('--delay',`${Math.random()*.18}s`);host.append(b);setTimeout(()=>b.remove(),1400)}}
...
function heroCaption(p){if(!p)return '';if(p.owner==='fish'){const f=data.fish.find(x=>x.id===p.ownerId);return `${f?f.name+' · ':''}${fmtDate(p.takenDate)}`}return `Tank · ${fmtDate(p.takenDate)}`}
function homeQuotes(){try{const custom=(localStorage.getItem(HOME_QUOTES_KEY)||'').split(/\n+/).map(q=>q.trim()).filter(Boolean).slice(0,40);return [...DEFAULT_HOME_QUOTES,...custom]}catch{return DEFAULT_HOME_QUOTES}}
function homeQuote(){const q=homeQuotes();return q[Math.floor(HOME_QUOTE_SEED*q.length)%q.length]||''}
...
function renderHome(){
  ...
  main.innerHTML=`<section class="screen home">
    ...
    ${homeQuote()?`<blockquote class="home-quote"><p>“${esc(homeQuote())}”</p><cite>Gene Wolfe · The Book of the New Sun</cite></blockquote>`:''}
  ...
  $('#atoRefilled').onclick=e=>{const btn=e.currentTarget,stat=btn.parentElement,strong=stat.querySelector('[data-count]'),record={id:uid(),date:today(),time:nowTime(),createdAt:timestamp()};burstBubbles(btn);upsertLocal('atoRefills',record);cloudSet('atoRefills',record);
...
// ---- Feed ----
function sortedFoodTypes(){return [...data.foodTypes].sort((a,b)=>a.name.localeCompare(b.name,undefined,{sensitivity:'base'}))}
function foodTypeFor(r){return data.foodTypes.find(f=>f.id===r.foodTypeId)}
function renderFeed(){const rows=[...data.feedings].sort(byNewest),foods=sortedFoodTypes();main.innerHTML=`<section class="screen stack">${screenHead('Feed','A log of what and when the tank was fed.')}<div class="panel food-types"><div class="detail-label-row"><div><strong>Food Types</strong><div class="muted subline">Create once, then select it when logging a feeding.</div></div><button type="button" class="button secondary" data-add-food-type>Add Food Type</button></div>${foods.length?`<div class="food-type-list">${foods.map(f=>`<article class="food-type-item">${avatar(latestPhoto('food',f.id),ICONS.bubbles)}<strong>${esc(f.name)}</strong>${rowActions('foodType',f.id,'food')}</article>`).join('')}</div>`:`<div class="muted food-type-empty">No reusable food types yet.</div>`}</div>${rows.length?`<div class="timeline">${rows.map(r=>{const ft=foodTypeFor(r),ph=ft&&latestPhoto('food',ft.id);return `<article class="timeline-item feed-entry">${ph?`<span class="feed-food-photo skeleton"><img class="fade-img" src="${esc(thumbSrc(ph))}" alt="" loading="lazy" crossorigin="anonymous"></span>`:''}<div class="feed-entry-copy"><time>${esc(fmtWhen(r.date,r.time))}</time><strong>${esc(r.food)}</strong>${r.notes?`<div class="muted">${esc(r.notes)}</div>`:''}</div><div class="timeline-actions">${rowActions('feed',r.id)}</div></article>`}).join('')}</div>`:empty('No feedings logged yet','Create a food type, then log what you feed the tank and when.')}</section>`;main.querySelector('[data-add-food-type]')?.addEventListener('click',()=>openEditor('foodType'));wireRows()}

// ---- Testing ----
const testTime = r => Date.parse(`${r.date}T${r.time||'12:00'}:00`);
function testingPanel(meta,rows){const all=testType==='all';return `<div class="panel test-panel"><div class="toolbar toolbar-split"><div><strong>${all?'All parameters':esc(meta[1])+' history'}</strong>${all?'':`<div class="muted subline">${esc(targetLabel(meta))}</div>`}</div><div class="chips">${[['3m','3 Months'],['1y','1 Year'],['3y','3 Years']].map(([r,l])=>`<button class="chip ${r===testRange?'active':''}" data-range="${r}">${l}</button>`).join('')}</div></div>${all?allTestChart(rows):testChart(rows,meta)}</div>`}
function testingList(meta,rows){const all=testType==='all';return `<div class="list test-list">${rows.length?rows.map(testRow).join(''):empty(all?'No readings yet':`No ${meta[1].toLowerCase()} readings`,all?'Add a measurement to begin comparing parameters.':`Add your first ${meta[1].toLowerCase()} measurement.`)}</div>`}
function renderTesting(){
  const all=testType==='all',meta=testMeta(all?'temperature':testType), rows=(all?data.tests:data.tests.filter(x=>x.type===testType)).sort(byNewest);
  main.innerHTML=`<section class="screen stack">${screenHead('Testing','One measurement at a time.')}
    <div class="chips testing-chips" id="testingChips"><button class="chip test-chip chip-all ${all?'active':''}" data-test-type="all">All</button>${TEST_TYPES.map(([id,label])=>`<button class="chip test-chip chip-${id} ${id===testType?'active':''}" data-test-type="${id}">${esc(label)}</button>`).join('')}</div>
    ${testingPanel(meta,rows)}${testingList(meta,rows)}</section>`;
  const chips=$('#testingChips');if(chips)chips.scrollLeft=testChipScroll;wireTesting();
}
function wireTesting(){const chips=$('#testingChips');if(chips)chips.onscroll=()=>{testChipScroll=chips.scrollLeft};main.querySelectorAll('[data-test-type]').forEach(b=>b.onclick=()=>{testType=b.dataset.testType;updateTesting()});main.querySelectorAll('[data-range]').forEach(b=>b.onclick=()=>{testRange=b.dataset.range;updateTesting()});wireRows()}
// Parameter/range changes update in place without moving the horizontally scrolled parameter strip.
function updateTesting(){const all=testType==='all',meta=testMeta(all?'temperature':testType),rows=(all?data.tests:data.tests.filter(x=>x.type===testType)).sort(byNewest),motion=!reducedMotion.matches;
  main.querySelectorAll('[data-test-type]').forEach(b=>b.classList.toggle('active',b.dataset.testType===testType));
...
function allTestChart(rows){const days={'3m':92,'1y':366,'3y':1096}[testRange],end=Date.now(),start=end-days*864e5,series=TEST_TYPES.map(meta=>{const points=rows.filter(r=>r.type===meta[0]).map(r=>({t:testTime(r),v:r.value,r})).filter(p=>p.t>=start).sort((a,b)=>a.t-b.t);if(!points.length)return null;const vals=points.map(p=>p.v),min=Math.min(...vals),max=Math.max(...vals),span=max-min||1;return {meta,points:points.map(p=>({...p,n:(p.v-min)/span}))}}).filter(Boolean);
  if(!series.length)return `<div class="empty empty-inset">No readings in this range.</div>`;const narrow=narrowQuery.matches,w=Math.max(280,Math.round(main.clientWidth-34)),h=narrow?210:250,pad=narrow?14:28,left=pad,x=t=>left+(t-start)/(end-start||1)*(w-left-pad),y=v=>h-pad-v*(h-2*pad),fmtDay=t=>new Date(t).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'});
  return `<div class="all-test-chart"><div class="all-test-legend">${series.map(s=>`<span><i style="--series:${s.meta[3]}"></i>${esc(s.meta[1])}</span>`).join('')}</div><svg class="chart${animateChart&&!reducedMotion.matches?' chart-animate':''}" viewBox="0 0 ${w} ${h}" role="img" aria-label="All water parameters overlaid"><line class="gridline" x1="${left}" y1="${pad}" x2="${left}" y2="${h-pad}"/><line class="gridline" x1="${left}" y1="${h-pad}" x2="${w-pad}" y2="${h-pad}"/>${series.map(s=>`${s.points.length>1?`<path class="series" stroke="${s.meta[3]}" pathLength="1" d="M${s.points.map(p=>`${x(p.t)},${y(p.n)}`).join(' L')}"/>`:''}${s.points.map(p=>`<circle class="dot" cx="${x(p.t)}" cy="${y(p.n)}" r="3.5" fill="${s.meta[3]}"><title>${esc(`${s.meta[1]} · ${fmtWhen(p.r.date,p.r.time)}: ${p.v}${s.meta[2]?' '+s.meta[2]:''}`)}</title></circle>`).join('')}`).join('')}<text x="${left}" y="${h-5}">${esc(fmtDay(start))}</text><text x="${w-pad}" text-anchor="end" y="${h-5}">Today</text></svg></div>`}
...
function fishAge(date){if(!date)return 'Acquisition date not set';const a=new Date(date+'T12:00:00'),b=new Date();let months=(b.getFullYear()-a.getFullYear())*12+b.getMonth()-a.getMonth();if(b.getDate()<a.getDate())months--;months=Math.max(0,months);const y=Math.floor(months/12),m=months%12;return `${y} ${y===1?'yr':'yrs'} ${m} ${m===1?'mo':'mos'}`} 
...
function renderFish(){const fish=sortedFish();if(!selectedFish())fishTab=fish[0]?.id||'';const sel=selectedFish(),showDetail=fishDetailShown(),showList=!narrowQuery.matches||!showDetail;
  main.innerHTML=`<section class="screen stack">${screenHead('Fish','Feeding and behavior notes by animal.')}<div class="master-detail">${showList?entityList(fish,{owner:'fish',fallbackIcon:ICONS.fish,selected:fishTab,dataAttr:'data-fish',meta:f=>fishAge(f.acquisitionDate),addLabel:'Add fish',addAttr:'data-add-fish'}):''}${showDetail?fishDetail(sel):(!narrowQuery.matches?empty('No fish yet','Add a fish to start its feeding and behavior notes.'):'')}</div></section>`;
...
// ---- Tank & Goals ----
function renderVisual(){const rows=[...data.tankVisual].sort(byNewest),latest=latestPhoto('visual');main.innerHTML=`<section class="screen stack">${screenHead('Tank','A dated visual journal of the tank.')}${latest?`<button type="button" class="tank-latest skeleton" data-photo="${esc(latest.id)}" aria-label="Open most recent tank photo"><img class="fade-img" src="${esc(fullSrc(latest))}" alt="Most recent tank photo" crossorigin="anonymous"><span>Most recent photo · ${esc(fmtDate(latest.takenDate))}</span></button>`:''}<div class="list">${rows.length?rows.map(r=>`<article class="row row-media"><div class="row-copy"><strong>${esc(fmtDate(r.date))}</strong>${r.notes?`<span>${esc(r.notes)}</span>`:''}${photoStrip('visual',r.id)}</div>${rowActions('visual',r.id)}</article>`).join(''):empty('No tank entries yet','Add a dated entry with notes, a photo, or both.')}</div></section>`;wireRows()}
...
async function openPhotoViewer(id){const p=data.photos.find(x=>x.id===id);if(!p)return;photoDialog.dataset.photoId=p.id;
  ...
  const img=photoDialog.querySelector('.photo-view img'),thumb=main.querySelector(`[data-photo="${CSS.escape(p.id)}"] img.fade-img`);enablePinch(img);enableCoralPhotoSwipe(p,img);markLoadedImages(photoDialog);
...
function enableCoralPhotoSwipe(p,img){if(p.owner!=='coral')return;const group=photosFor('coral',p.ownerId),index=group.findIndex(x=>x.id===p.id);if(index<0||group.length<2)return;let start=null;const frame=img.closest('.photo-frame');frame.addEventListener('touchstart',e=>{if(e.touches.length!==1||img.classList.contains('zoomed'))return;start={x:e.touches[0].clientX,y:e.touches[0].clientY}},{passive:true});frame.addEventListener('touchend',e=>{if(!start||img.classList.contains('zoomed')){start=null;return}const t=e.changedTouches[0],dx=t.clientX-start.x,dy=t.clientY-start.y;start=null;if(Math.abs(dx)<55||Math.abs(dx)<Math.abs(dy)*1.25)return;const next=group[index+(dx<0?1:-1)];if(!next)return;photoDialog.close();requestAnimationFrame(()=>openPhotoViewer(next.id))},{passive:true})}
...
function openEditor(kind,item=null,ctx={}){const editing=!!item,id=item?.id||uid();let title='',fields='';
  ...
  if(kind==='feed'){const foods=sortedFoodTypes(),selected=item?.foodTypeId||'';title=editing?'Edit feeding':'Log feeding';fields=field('Food type',`<select name="foodTypeId" required><option value="">${foods.length?'Choose a food type':'Create a food type first'}</option>${foods.map(f=>`<option value="${esc(f.id)}" ${f.id===selected?'selected':''}>${esc(f.name)}</option>`).join('')}${editing&&!selected&&item?.food?`<option value="legacy" selected>${esc(item.food)}</option>`:''}</select>`)+field('Date',dateInput('date',item?.date||today()))+field('Time',`<input name="time" type="time" value="${esc(editing?item.time:nowTime())}">`)+field('Notes',`<textarea name="notes" maxlength="500" placeholder="Optional">${esc(item?.notes||'')}</textarea>`,'full')}
  if(kind==='foodType'){title=editing?'Edit food type':'Add food type';fields=field('Name',`<input name="name" maxlength="120" required value="${esc(item?.name||'')}" autocomplete="off">`,'full')}
  if(kind==='fish'){title=editing?'Edit fish':'Add fish';fields=field('Name',`<input name="name" maxlength="80" required value="${esc(item?.name||'')}" autocomplete="off">`)+field('Date acquired',dateInput('acquisitionDate',item?.acquisitionDate,false))}
...
editorForm.onsubmit=async e=>{e.preventDefault();const fd=new FormData(editorForm),kind=editorForm.dataset.kind,id=editorForm.dataset.id,ctx=JSON.parse(editorForm.dataset.ctx||'{}'),now=timestamp(),old=findKind(kind,id),collection=KIND_COLLECTION[kind];let record=null;if(kind==='visual'&&!safeText(fd.get('notes'),2000)&&!pendingFiles.length&&!photosFor('visual',id).length){editorForm.querySelector('.dialog-hint').textContent='Add notes, a photo, or both.';return}
  ...
  if(kind==='feed'){const foodTypeId=String(fd.get('foodTypeId')||''),type=data.foodTypes.find(f=>f.id===foodTypeId);record={id,foodTypeId:type?.id||old?.foodTypeId||'',food:type?.name||old?.food||'',date:safeDate(fd.get('date'))||today(),time:safeTime(fd.get('time')),notes:safeText(fd.get('notes'),500),createdAt:old?.createdAt||now};if(!record.food)return}
  if(kind==='foodType')record={id,name:safeText(fd.get('name'),120),createdAt:old?.createdAt||now};
  if(kind==='fish'){record={id,name:safeText(fd.get('name'),80),acquisitionDate:safeDate(fd.get('acquisitionDate')),createdAt:old?.createdAt||now};if(!old){fishTab=id;fishDetailOpen=true}}
...
function performDelete(kind,id){const collection=KIND_COLLECTION[kind],item=findKind(kind,id);if(!collection||!item)return;
  ...
  const photoOwner={fish:'fish',coral:'coral',visual:'visual',goal:'goal',foodType:'food'}[kind];if(photoOwner)related.push(...photosFor(photoOwner,id).map(p=>['photos',p]));
...
function wireAutoGrow(root,maxHeight=600){root.querySelectorAll('textarea').forEach(t=>{const grow=()=>{t.style.height='auto';const height=Math.min(t.scrollHeight,maxHeight);t.style.height=height+'px';t.style.overflowY=t.scrollHeight>maxHeight?'auto':'hidden'};t.addEventListener('input',grow);grow()})}
...
function openSettings(){let customQuotes='';try{customQuotes=localStorage.getItem(HOME_QUOTES_KEY)||''}catch{}$('#settingsContent').innerHTML=`${closeButton()}<h2>Settings</h2><section class="settings-section"><h3>Tank</h3><p>Volume: ${TANK_GALLONS} gallons</p></section><section class="settings-section"><h3>Appearance</h3><label class="setting-toggle"><span>Water shimmer on the Home photo<small>A slow ripple of light across the tank photo. Saved on this device.</small></span><input type="checkbox" switch id="shimmerToggle" ${shimmerOn()?'checked':''}></label><label class="field quote-setting"><span>Book of the New Sun quotes<small>Add your own short excerpts, one per line. Home randomly rotates through them with the built-in quote.</small></span><textarea id="homeQuotesInput" maxlength="4000" placeholder="One short quote per line">${esc(customQuotes)}</textarea></label></section><section class="settings-section"><h3>Backup & restore</h3><p>Download a copy of all your CoralDar data, or restore from one. Restoring replaces what’s in the app now. Photos stay in Cloudinary; the backup keeps their links.</p><div class="toolbar"><button class="button secondary" id="backupButton">Download backup</button><button class="button secondary" id="restoreButton">Restore backup</button><input id="restoreFile" type="file" accept="application/json,.json" hidden></div><p id="backupStatus" class="settings-status" role="status"></p></section><section class="settings-section"><h3>Account</h3><p id="syncText">${syncLabel()}</p><button class="button secondary" id="signOutButton">Sign out</button><p>CoralDar release ${RELEASE}</p></section>`;settingsDialog.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>dismiss(settingsDialog));$('#shimmerToggle').onchange=e=>{try{localStorage.setItem(SHIMMER_KEY,e.target.checked?'on':'off')}catch{}renderSkipped=true};wireAutoGrow(settingsDialog,800);$('#homeQuotesInput').onchange=e=>{try{localStorage.setItem(HOME_QUOTES_KEY,e.target.value.slice(0,4000))}catch{}renderSkipped=true};$('#backupButton').onclick=downloadBackup;$('#restoreButton').onclick=()=>$('#restoreFile').click();$('#restoreFile').onchange=restoreBackup;$('#signOutButton').onclick=signOutUser;settingsDialog.showModal()}
